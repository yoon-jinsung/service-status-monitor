...
팀이 의존하는 외부 SaaS 서비스(Notion, GitHub, Vercel, Slack 등)의 장애 및 상태 변화를 **자동으로 감지**하여 **Slack 채널에 실시간 알림**을 전달하는 모니터링 서비스입니다.
...
flowchart TB
    ...
    config -->|설정 로드| cron
    cron -->|"1. 상태 API 폴링"| external
    external -->|응답| cron
    state <-->|"2. 이전 상태 비교"| cron
    cron -->|"3. 변화 감지 시 알림"| slack
    cron -->|"4. 이력 기록"| logs
...
| **폴링 주기** | 5분 | GitHub Actions cron 최소 단위 |
...
| **Notion** | `https://status.notion.so` | `/api/v2/status.json` | Atlassian Statuspage |
| **Vercel** | `https://www.vercel-status.com` | `/api/v2/status.json` | Atlassian Statuspage |
  ...
  check-status:
    ...
    steps:
      ...
      - name: Check service status
        run: node tools/status-monitor/check.js
        env:
          SLACK_WEBHOOK_URL: ${{ secrets.SLACK_STATUS_WEBHOOK_URL }}
...
```
1. GitHub Actions가 5분마다 워크플로우 트리거
2. config/services.json에서 모니터링 대상 로드
3. 각 서비스 상태 API를 병렬 호출 (Promise.allSettled)
4. state/status.json에서 이전 상태 로드
5. 현재 상태와 이전 상태 비교
6. 변화 감지 시:
   a. Slack Webhook으로 알림 전송
   b. logs/incidents.json에 이력 추가
7. state/status.json 업데이트 후 Git 커밋 & 푸시
```
  ...
  for (const [name, current] of Object.entries(currentResults)) {
    ...
    if (!previous) {
      if (current.indicator !== 'none') {
        changes.push({ service: name, previous: 'none', current: current.indicator });
      }
      continue;
    }
...
tools/status-monitor/
  ...
  src/
    ...
    poller.js             # 상태 API 폴링
    parsers/
      statuspage.js       # Atlassian Statuspage 파서
      slack.js            # Slack 독자 API 파서
    detector.js           # 상태 변화 감지
    notifier.js           # Slack Webhook 알림
    state.js              # state/status.json 읽기/쓰기
    logger.js             # logs/incidents.json 이력 기록
...
| Statuspage API 폴링 + 파서 구현 | `poller.js`, `parsers/statuspage.js` |
| 상태 변화 감지 로직 | `detector.js` |
| Slack Webhook 알림 전송 | `notifier.js` |
| state/status.json 읽기/쓰기 | `state.js` |
| GitHub Actions 워크플로우 | `.github/workflows/status-monitor.yml` |
| Slack App 생성 + Webhook URL 설정 | GitHub Secrets |

**MVP 범위**: GitHub/Notion/Vercel 3개 서비스 상태 모니터링 + 장애/복구 Slack 알림

### Phase 2: 고도화 (3~4일)

| 작업 | 설명 |
|------|------|
| Slack 독자 API 파서 | `parsers/slack.js` |
